...
- Backend: signed, random session identifiers in HTTP-only cookies; OAuth tokens, CSV rows and current batch metadata in temporary process memory; bounded upload files in a private OS temp directory.
...
- Limits: 50 MB ZIP, 250 MB total expanded, 15 MB per certificate, 200:1 compression ratio, 2,000 archive entries, 1,000 certificate files, 2 MB CSV, 1,000 CSV recipients. These are application ceilings, not a guarantee that your selected hosting plan supports every maximum simultaneously.
